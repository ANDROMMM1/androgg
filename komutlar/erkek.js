...
var toplam = db.fetch(`toplamKayit_${message.author.id}`)
 ...
 const genelrol = message.guild.roles.find(r => r.id === "774277124986503219");
  const erkek = message.guild.roles.find(r => r.id === "774277124986503219");
  ...
  if(!message.member.roles.array().filter(r => r.id === "774277116384378900")[0]) { 
    ...
    .addField("<a:developer:786657839615705108> __Bir Erkek Kaydı Yapıldı__ <a:developer:786657839615705108>", "```Sunucumuza Bir Erkek Vatandaşımızın Kaydı Yapıldı. Lütfen İçeride Ona Saygılı Bir Aile Ortamı Kuracak Şekilde Yaklaşın!.```")
    .addField(`<a:onayli:777840751429681163> Kayıdı Gerçekleştirilen __Erkek__ Vatandaşımız ==\n`, `**__${c.user.tag}__**`)
